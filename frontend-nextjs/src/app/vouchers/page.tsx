...
export default function VouchersPage() {
  ...
  const handleCancelVoucher = async () => {
    ...
    setIsDeleting(true)
    try {
      const res = await fetch(`${API_BASE}/vouchers/${deletingVoucher.voucher_id}/cancel`, {
        method: 'POST',
        headers: authHeaders(token)
      })
      const resData = await res.json()
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            <div className="flex items-center gap-3 text-rose-600 dark:text-rose-400">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Manage Voucher #{deletingVoucher.voucher_number}</h3>
