...
export default function EditOrderPage({ params }: EditProps) {
  ...
  // Load cache AND existing order details
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [lsRes, ssRes, orderRes] = await Promise.all([
          fetch(`${API_BASE}/ledgers`, { headers }),
          fetch(`${API_BASE}/inventory/items`, { headers }),
          fetch(`${API_BASE}/temporders/${orderId}`, { headers })
        ])
        ...
        setCachedProducts(Array.isArray(ss) ? ss : [])
        ...
        if (orderRes.ok && orderData) {
          ...
          if (orderData.ledger_id) {
            setIsCustomShop(false)
            setSelectedShop(customers.find((c: any) => c.ledger_id === orderData.ledger_id) || { ledger_id: orderData.ledger_id, name: orderData.customer_name })
            setShopQuery(toTitleCase(orderData.customer_name))
            setCustomShopHasGst(false)
            setCustomShopGstin('')
          } else {
            setIsCustomShop(true)
            setCustomShopName(orderData.custom_customer_name || '')
            const gstin = (orderData.custom_customer_gstin || '').trim().toUpperCase()
            setCustomShopGstin(gstin)
            setCustomShopHasGst(Boolean(gstin))
          }

          setCart(orderData.items.map((it: any) => {
            const matchedProd = it.stock_item_id && Array.isArray(ss) ? ss.find((p: any) => p.item_id === it.stock_item_id) : null
            const company = it.company_name || getCompanySuffix(matchedProd)
            return {
              cartItemId: Math.random().toString(36).substring(2, 9),
              stock_item_id: it.stock_item_id || null,
              custom_item_name: it.custom_item_name || (!it.stock_item_id ? it.stock_item_name : undefined),
              name: it.stock_item_name,
              company_name: company,
              qty: it.qty,
              price: it.price,
              is_bill_required: it.is_bill_required ?? it.has_gst ?? true,
              is_custom: Boolean(!it.stock_item_id || it.is_custom || it.custom_item_name)
            }
          }))
        } else {
          router.push('/temporders')
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/temporders/${orderId}`, {
        method: 'PUT',
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 max-w-xl mx-auto w-full space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">Modify items, rates, or outlet details</p>
          </div>
        </div>
        {/* Step Indicators */}
