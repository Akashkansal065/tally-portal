...
class Employee(Base):
    ...
    linked_user_id = Column(Integer, ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True)
    ...
    payment_ledger_id = Column(Integer, ForeignKey("ledgers.ledger_id"), nullable=False)
...
class SalaryComponent(Base):
    ...
    linked_ledger_id = Column(Integer, ForeignKey("ledgers.ledger_id"), nullable=False)
    
    linked_ledger = relationship("Ledger")
...
class SalaryStructureComponent(Base):
    ...
    structure_id = Column(Integer, ForeignKey("salary_structures.structure_id", ondelete="CASCADE"), nullable=False)
...
class Payslip(Base):
    ...
    period_id = Column(Integer, ForeignKey("payroll_periods.period_id", ondelete="CASCADE"), nullable=False)
    ...
    voucher_id = Column(BigInteger, ForeignKey("vouchers.voucher_id", ondelete="SET NULL"), nullable=True)
    payment_voucher_id = Column(BigInteger, ForeignKey("vouchers.voucher_id", ondelete="SET NULL"), nullable=True)
    generated_at = Column(DateTime, server_default=func.now())
    
    period = relationship("PayrollPeriod")
    employee = relationship("Employee")
    voucher = relationship("Voucher", foreign_keys=[voucher_id])
    payment_voucher = relationship("Voucher", foreign_keys=[payment_voucher_id])
...
class PayslipComponent(Base):
    ...
    payslip_id = Column(BigInteger, ForeignKey("payslips.payslip_id", ondelete="CASCADE"), nullable=False)
    component_id = Column(Integer, ForeignKey("salary_components.component_id"), nullable=False)
    amount = Column(Numeric(18, 2), nullable=False)
