...
class PaymentLink(Base):
    ...
    company_id = Column(Integer, ForeignKey("companies.company_id", ondelete="CASCADE"), nullable=False, index=True)
    bill_id = Column(BigInteger, ForeignKey("bills.bill_id", ondelete="CASCADE"), nullable=False)
    gateway_config_id = Column(Integer, ForeignKey("payment_gateway_configs.gateway_config_id"), nullable=False)
...
class GatewayTransaction(Base):
    ...
    company_id = Column(Integer, ForeignKey("companies.company_id", ondelete="CASCADE"), nullable=False, index=True)
    payment_link_id = Column(BigInteger, ForeignKey("payment_links.payment_link_id", ondelete="SET NULL"), nullable=True)
    bill_id = Column(BigInteger, ForeignKey("bills.bill_id"), nullable=False)
    gateway_config_id = Column(Integer, ForeignKey("payment_gateway_configs.gateway_config_id"), nullable=False)
...
class WebhookEvent(Base):
    ...
    gateway_config_id = Column(Integer, ForeignKey("payment_gateway_configs.gateway_config_id", ondelete="CASCADE"), nullable=False)
